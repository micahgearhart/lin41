...
## MAKE A Master Table with MAPQ FLAG and Biotype
...
d<-res_dds87q4[,]
plot(d[,3],d[,2],cex=0.5,pch=16,ylim=c(-7,7),xlim=c(-7,7),main="Oma-1 vs Lin-41 Enrichment", 
     xlab="LIN-41 Log2Fold Change",ylab="OMA-1 Log2Fold Change",
...
table(degs_lin41<-as.numeric(expressed_genes %in% lin41_genesq4_exclusive ))
...
table(degs_oma1<-as.numeric(expressed_genes %in% oma1_genesq4_exclusive ))
...
idx<-match(rownames(res),anno87$ensembl_gene_id)
res$name<-anno87[idx,"wikigene_name"]
res$external_name<-anno87[idx,"external_gene_name"]
res$type<-anno87[idx,"transcript_biotype"]
head(res,20)
...
## Export Master Table and FPKM table
...
```{r exportFPKM2,eval=F}
#add annotations to f
idx<-match(rownames(f),anno87$ensembl_gene_id)
f$name<-anno87[idx,]$external_gene_name
head(f)
...
idx<-match(rownames(temp),anno87$ensembl_gene_id)
temp$name<-anno87[idx,]$external_gene_name
pheatmap(temp[,1:3],labels_row=anno87[idx,]$external_gene_name)
